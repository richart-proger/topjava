...
public class JdbcMealRepositoryForHsqldb extends AbstractJdbcMealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                ...
                .addValue("date_time", DateTimeUtil.toTimeStamp(meal.getDateTime()))
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        String startDt = DateTimeUtil.toTimeStamp(startDateTime);
        String endDt = DateTimeUtil.toTimeStamp(endDateTime);

        return super.getJdbcTemplate().query(
                "SELECT * FROM meals WHERE user_id=?  AND date_time >=  ? AND date_time < ? ORDER BY date_time DESC",
                AbstractJdbcMealRepository.getRowMapper(), userId, startDt, endDt);
    }
}
