...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.ALL_SORTED_BETWEEN_HALF_OPEN, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.user.id=?1  AND m.dateTime >=  ?2 AND m.dateTime < ?3 ORDER BY m.dateTime DESC")
})
@Entity
@Table(name = "meals",
